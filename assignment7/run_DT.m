...
default_error= sum(abs(y_pred-y_test))/size(y_test,1)
...
new_tree=fitctree(x,y,'MaxNumSplits',50, 'MinParentSize',20,'MinLeafSize',5, 'MergeLeaves','on','splitcriterion','gdi');
%%% show the tree logic
new_tree=prune(new_tree, 'level',1);
view(new_tree);
%%% show the graphics of the tree
