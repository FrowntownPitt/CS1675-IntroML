...
for i=1:10
    for j=1:20
        [t_y] = Bag_classifier(tr_x,tr_y,test_x,sprintf('[@SVML_base,%d,[]]',i));
        err_test_bag(i) = err_test_bag(i) + sum(t_y == test_y)/size(test_y,1);
        [tr_y2] = Bag_classifier(tr_x,tr_y,tr_x,sprintf('[@SVML_base,%d,[]]',i));
        err_train_bag(i) = err_train_bag(i) + sum(tr_y2 == tr_y)/size(tr_y,1);
        
        [t_y] = Boost_classifier(tr_x,tr_y,test_x,sprintf('[@SVML_base,%d,[]]',i));
        err_test_boost(i) = err_test_boost(i) + sum(t_y == test_y)/size(test_y,1);
        [tr_y2] = Boost_classifier(tr_x,tr_y,tr_x,sprintf('[@SVML_base,%d,[]]',i));
